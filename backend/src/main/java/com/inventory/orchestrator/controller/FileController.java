...
public class FileController {
    ...
    public ResponseEntity<ApiResponse<BulkImportResult>> bulkUpload(
        ...
        try {
            ...
            FileMappingConfigDTO transferMapping = objectMapper.readValue(transferMappingJson,
                    FileMappingConfigDTO.class);

            BulkImportResult result = dataImportService.importBulkData(
                    stockFile, stockMapping,
                    salesFile, salesMapping,
                    transferFile, transferMapping);

            if (result.getSuccess()) {
                return ResponseEntity.ok(ApiResponse.success(result, "All files processed successfully"));
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            ...
            errorResult.setMessage("Bulk upload failed: " + e.getMessage());
            errorResult.setGlobalErrors(List.of(e.getMessage()));
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.success(errorResult, "Failed to process bulk upload"));
        }
    }
     ...
     * After validation passes, users should upload files separately using /upload
     * endpoint.
    ...
    public ResponseEntity<ApiResponse<StockConsistencyValidationResult>> validateConsistencyOnly(
            ...
            @RequestParam("salesMapping") String salesMappingJson,
            @RequestParam("transferMapping") String transferMappingJson) {
        // This is the same as validate-consistency endpoint, kept for backward
        // compatibility
