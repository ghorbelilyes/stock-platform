...
import com.inventory.orchestrator.dto.BulkImportResult;
...
public class DataImportService {
    ...
    public BulkImportResult importBulkData(
            MultipartFile stockFile, FileMappingConfigDTO stockMapping,
            MultipartFile salesFile, FileMappingConfigDTO salesMapping,
            MultipartFile transferFile, FileMappingConfigDTO transferMapping) throws Exception {

        BulkImportResult bulkResult = new BulkImportResult();
        bulkResult.setSuccess(true);

        // 1. Perform consistency validation first
        ...
        if (!validationResult.getValid()) {
            ...
            bulkResult.setMessage("Stock consistency validation failed");
            bulkResult.setGlobalErrors(validationResult.getErrors());
            return bulkResult;
        }

        // 2. Process Stock Import
        ImportResult stockResult = importStockData(stockFile, stockMapping);
        bulkResult.getResults().put("stock", stockResult);
        if (!stockResult.getValid()) {
            bulkResult.setSuccess(false);
            throw new RuntimeException("Stock file contains errors: " + stockResult.getErrors().get(0));
        }

        // 3. Process Sales Import
        ImportResult salesResult = importSalesData(salesFile, salesMapping);
        bulkResult.getResults().put("sales", salesResult);
        if (!salesResult.getValid()) {
            bulkResult.setSuccess(false);
            throw new RuntimeException("Sales file contains errors: " + salesResult.getErrors().get(0));
        }

        // 4. Process Transfer Import
        ImportResult transferResult = importTransferData(transferFile, transferMapping);
        bulkResult.getResults().put("transfer", transferResult);
        if (!transferResult.getValid()) {
            bulkResult.setSuccess(false);
            throw new RuntimeException("Transfer file contains errors: " + transferResult.getErrors().get(0));
        }

        bulkResult.setMessage("All files uploaded and processed successfully");
        return bulkResult;
    }
    ...
    public ImportResult importStockData(MultipartFile file, FileMappingConfigDTO mappingConfig) throws Exception {
        ...
        for (Map<String, String> csvRow : csvRows) {
            ...
            try {
                ...
                Stock stock = stockRepository.findByIdStoreAndIdProduct(idStore, idProduct);
                if (stock != null) {
        ...
        if (!stocksToSave.isEmpty()) {
            ...
            result.setRowsInserted(insertedCount);
            result.setRowsUpdated(updatedCount);
        }
    ...
    public ImportResult importSalesData(MultipartFile file, FileMappingConfigDTO mappingConfig) throws Exception {
        ...
        for (Map<String, String> csvRow : csvRows) {
            ...
            try {
                ...
                // Check if sales record already exists (same store, product, and exact
                // timestamp)
     ...
     * Supports updating existing transfers (by matching date, stores, product) or
     * creating new ones
     * When status changes from "approved" to "in_transit", reduces stock from
     * sending store
     * When creating new transfers with status "in_transit", reduces stock from
     * sending store
     */
    @Transactional
    public ImportResult importTransferData(MultipartFile file, FileMappingConfigDTO mappingConfig) throws Exception {
        ...
        for (Map<String, String> csvRow : csvRows) {
            ...
            try {
                ...
                // - in_transit -> received: add to receiving store (sending store already
                // reduced)
                ...
                if (existingTransfer == null) {
                    ...
                    if ("in_transit".equals(status)) {
                        ...
                        // New transfer with "received"/"closed" status - reduce from sending store AND
                    ...
                    if ("approved".equals(oldStatus) && "in_transit".equals(status)) {
                        ...
                        // approved -> received/closed: reduce from sending store AND add to receiving
                        // store
                        reduceStockForTransfer(idStoreSent, idProduct, quantity, result, rowNumber);
                        addStockForTransfer(idStoreReceive, idProduct, quantity, result, rowNumber);
                    } else if ("in_transit".equals(oldStatus)
                            && ("received".equals(status) || "closed".equals(status))) {
                        // in_transit -> received/closed: add to receiving store (sending store already
                        // reduced)
                        addStockForTransfer(idStoreReceive, idProduct, quantity, result, rowNumber);
                    ...
                    } else if (("received".equals(oldStatus) || "closed".equals(oldStatus))
                            && "in_transit".equals(status)) {
                        // received/closed -> in_transit: remove from receiving store (sending store was
                        // already reduced)
                    ...
                    // Other status changes (e.g., approved -> approved, received -> received): no
                    // stock change
                }
     ...
     */
    private void reduceStockForTransfer(Long idStoreSent, Long idProduct, Integer quantity, ImportResult result,
            int rowNumber) {
        try {
            Stock stock = stockRepository.findByIdStoreAndIdProduct(idStoreSent, idProduct);
            if (stock == null) {
                result.getErrors().add(
                        "Row " + rowNumber + ": No stock found for Store " + idStoreSent + ", Product " + idProduct);
                return;
            }
     ...
     */
    private void addStockForTransfer(Long idStoreReceive, Long idProduct, Integer quantity, ImportResult result,
            int rowNumber) {
        try {
    ...
    private FileUpload createFileUploadRecord(MultipartFile file, FileType fileType, FileMappingConfigDTO mappingConfig)
            throws Exception {
    ...
    private void updateFileUploadRecord(FileUpload fileUpload, ImportResult result) {
        ...
        fileUpload.setRowsUpdated(result.getRowsUpdated());
