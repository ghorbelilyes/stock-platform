...
FROM (VALUES
    ...
    ('1003', 'Product 3', 'Third test product', 1),
    ('1004', 'Product 4', 'Fourth test product', 1)
...
SELECT 
    CAST(v.range_date AS TIMESTAMP),
    v.id_store,
    v.id_product,
    v.quantity
FROM (VALUES
    -- Historical sales for testing
    ('2025-01-14 10:00:00', 100, 1001, 5),   -- Store A - Product 1
    ('2025-01-14 11:00:00', 100, 1002, 3)    -- Store A - Product 2
) AS v(range_date, id_store, id_product, quantity)
WHERE NOT EXISTS (
    SELECT 1 FROM sales 
    WHERE sales.range_date = CAST(v.range_date AS TIMESTAMP)
...
FROM (VALUES
    ...
    ('2025-01-14 08:00:00', 200, 100, 1001, 'Initial restock', 20, 'received'),  -- Warehouse B -> Store A, Product 1
    ('2025-01-13 09:00:00', 200, 100, 1002, 'Initial restock', 15, 'received'),  -- Warehouse B -> Store A, Product 2
    ('2025-01-15 08:00:00', 200, 100, 1001, 'Restock', 25, 'in_transit')         -- Warehouse B -> Store A, Product 1 (in_transit)
) AS v(date, id_store_sent, id_store_receive, id_product, reason, quantity, status)
WHERE NOT EXISTS (
    SELECT 1 FROM transfer 
    WHERE transfer.date = CAST(v.date AS TIMESTAMP)
